...
public:
	...
	S qry(int l, int r) {
		...
		if (l > r) { return S(); }
		S sml = S(), smr = S();
		for (int lt = l + _n - 1, rt = r + _n; lt < rt; lt >>= 1, rt >>= 1) {
...
private:
	...
	void up(int k) {
		for (int kt = (k + _n - 1) >> 1; kt; kt >>= 1) { pushup(kt); }
	}
};
...
public:
	...
	S qry(int l, int r) {
		...
		if (l > r) { return S(); }
		S sml = S(), smr = S();
		int lp = (l + _n - 1) >> 1, rp = (r + _n - 1) >> 1;
		for (int lt = l + _n - 1, rt = r + _n; lt < rt; lt >>= 1, rt >>= 1, lp >>= 1, rp >>= 1) {
			if (lt & 1) { sml = sml + tr[lt++]; }
			if (rt & 1) { smr = tr[--rt] + smr; }
